...
error MACT6340Contract__MaxSupplyReached();
error MACT6340Contract__ValueNotEqualPrice();
error MACT6340Contract__WrongAvenueForThisTransaction();

/// @custom:security-contact spencernjones@gmail.com
contract MACT6340Contract is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Burnable,
    ...
    ReentrancyGuard
{
    ...
    receive() external payable {
        revert MACT6340Contract__WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert MACT6340Contract__MaxSupplyReached();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert MACT6340Contract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
